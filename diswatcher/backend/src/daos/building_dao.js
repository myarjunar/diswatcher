class BuildingDao {
  ...
  async findByBbox(bboxObj) {
    const bboxQuery = `
      ...
      WHERE st_intersects(osm_buildings.geometry, 'SRID=4326;POLYGON((
  ...
  async findByBoundary(geometry) {
    const boundaryQuery = `
      SELECT * FROM osm_buildings
      WHERE st_intersects(osm_buildings.geometry, '${geometry}');
    `;
  ...
  async intersection(geometry) {
    const intersectionQuery = `
      SELECT st_intersection(osm_buildings.geometry, '${geometry}') as geometry, st_area(geometry) as area, name, changeset_user as user
      FROM osm_buildings
      WHERE st_intersects(osm_buildings.geometry, '${geometry}');
    `;
